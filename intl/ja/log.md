...
## 2022/09/19 day99
* 4.5まで進んだ段階で用語と各章の関連性についてまとめた
* ある程度大事な部分がわかって大まかな流れがわかったので、コードを書くより学ぶことに注力できた感じがある

## 2022/09/20 day100
* Pythonで米国株について学んだ
* やっぱり実用的な内容を同時に学ぶと定着する感じある
* 100日コード各生活完了！

## 2022/09/21 day101
* 相場の把握ツールを作ろうとした。
* イールドカーブってなんや！意味も作り方もわからん。
* 複雑な図を作らなければpandasで十分では？

## 2022/09/22 day102
* 牛尾剛さんの記事読んでちゃんと理解して使えるようになろうマインドになった。
* とりあえず位置からDraft.jsのドキュメント読んで見る

## 2022/09/23 day103
* エンティティやリッチスタイリングがどういう位置づけでどういう振る舞いをするかを学習した。
* 腰を据えてドキュメントを読むと理解度が違うし、よくわからないという感覚が少なくていい感じ。

## 2022/09/24 day104
* デコレータとブロック周りを活用すればインライン上にテンプレートを作成できるのでは？みたいな仮設を立てた。
* 次回はまず、ブロック上のデコレータの簡単な実験からやってみよう

## 2022/09/25 day105
* デコレータとブロック周りを活用すればインライン上にテンプレートを作成できるのでは？みたいな仮設を立てた。
* デコレータの文字列がクリックできることがわかった
* テンプレートの文字列からスクリプトを作成して実行する。新規ブロックに追加する内容を今後作る
...
## 2022/10/02 day111
- 課題について考えているうちに、結局どのような作業に関して集中して取り組むのか？というところのヒントを得た。

## 2022/10/04 day112
- コードを書くよりも課題を解決する方向に向いてきた。
- コードを書く練習時間を別途取らないと行けないような気がする。
